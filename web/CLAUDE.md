...
| What | Where |
|---|---|
| Brand color seed | `pnpm gen:colors '#NEWHEX'` → paste output into the COLOR section of `src/styles/tokens.css`. |
| Site identity | `src/site.config.ts` — single source of truth for SEO and JSON-LD. |
| Logo files | `public/logo.svg`, `public/logo-dark.svg`, `public/favicon*`, `public/apple-touch-icon.png`. |
| Layout chrome | `src/layouts/Layout.astro` (nav links, theme color). |
| Components | `src/components/` — pick new 2-letter prefixes per component. |
