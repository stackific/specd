...
func TestStripCriteriaSectionStripsH1Title(t *testing.T) {
	body := strings.Join([]string{
		"# My Task Title",
		"",
		"## Description",
	...
	got := stripCriteriaSection(body)
	if strings.Contains(got, "# My Task Title") {
		t.Errorf("H1 title should be removed (page header renders it), got:\n%s", got)
	}
	if !strings.Contains(got, "The body text.") {
		t.Errorf("body should be preserved, got:\n%s", got)
	}
	// H2 with same prefix character should NOT be confused for H1.
	body2 := "## Section\n\nbody"
	if !strings.Contains(stripCriteriaSection(body2), "## Section") {
		t.Errorf("H2 should be preserved")
	}
}
...
// rewriteTaskFile — i.e., the filesystem stays in sync after a move.
