// handlers_specs_page.go implements the GET /specs page handler. Renders the
// project's specs in one of three views (grouped table, grouped cards, flat
// table), all paginated.
package cmd
...
// SpecsPageData is the view model passed to the specs page template.
...
// makeSpecsHandler returns an http.HandlerFunc that renders /specs.
func makeSpecsHandler(freshPages func() map[string]*template.Template, devMode bool, cssHash, jsHash string) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		// Resolve the type filter against configured spec types so user input
		// can never reach SQL or templates as raw text.
		...
		if err := loadSpecsPage(&data); err != nil {
			slog.Error("specs page", "error", err)
			http.Error(w, "failed to load specs: "+err.Error(), http.StatusInternalServerError)
			return
		}

		renderPage(w, r, freshPages(), "specs", &PageData{
			Title:   "Specs",
			Active:  "specs",
			DevMode: devMode,
			CSSHash: cssHash,
...
// empty slice if the project config can't be read (handler still renders).
...
func loadSpecsPage(data *SpecsPageData) error {
	...
	data.PageNums = pageWindow(data.Page, data.TotalPages, 5)
	return nil
}
